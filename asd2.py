...
def load_data(timeframe):
    file_path = f"SET_SCC_{timeframe}.csv"
    data = pd.read_csv(file_path, parse_dates=["date"])
    # แปลงคอลัมน์ 'date' จาก string เป็น datetime
...
def calculate_returns(data,data1):
    data['return'] = data['close'].pct_change()
    data1['return'] = data['close'].pct_change()
...
def main():
    ...
    timeframe = st.sidebar.radio("Select Timeframe", ["1D", "4H"], index=0)
    ...
    data1 = load_data(timeframe)
    data,data1 = calculate_returns(data,data1)
    data, mu, sigma = detect_outliers(data)
    
    st.write("### 📌 Outliers Found:")
    st.dataframe(data[data['outlier']], height=300)

     # แสดงข้อมูล Outliers ทั้งหมด
    st.write("### 📌 Outliers All Data:")
    st.dataframe(data1[['return']], height=500)
